...
function HomeContent() {
  ...
  const [activeTab, setActiveTab] = useState<"my-learning" | "all-courses">(
    "my-learning",
  );
  ...
  useEffect(() => {
    ...
    setRegisteredIds(
      savedList ? JSON.parse(savedList) : ["computer-architecture"],
    );
    setCompletedIds(
  ...
  const allDisplayedCourses =
    selectedCategory === "Tất cả"
      ? mockCourses
      : mockCourses.filter(
          (c) =>
            c.category === selectedCategory ||
            c.instructor === selectedCategory,
        );
          ...
          <div className="space-y-6 animate-in fade-in duration-150">
            <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-2xs">
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">
                Danh sách các bài học bạn đã đăng ký tham gia trên hệ thống
                LUMER.
              </p>
            </div>
            ...
            {myCourses.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {myCourses.map((course) => {
                  ...
                  const currentProgress =
                    course.id === "computer-architecture" ? 4 : 0;

                  return (
                    <div
                      key={course.id}
                      className={`bg-white border rounded-2xl p-5 flex flex-col justify-between shadow-2xs transition ${isCompleted ? "border-emerald-200 bg-emerald-50/5" : "border-gray-200"}`}
                    >
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span
                            className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-wide ${isCompleted ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-[#0066FF]"}`}
                          >
                            {course.category}
                          </span>
                        ...
                        <h3 className="text-xs font-black text-gray-900 line-clamp-2 min-h-[32px] leading-tight">
                          {course.title}
                        </h3>
                      ...
                      <div className="mt-6 pt-3 border-t border-gray-100 flex justify-between items-center">
                        <span className="text-[10px] text-gray-400 font-bold">
                          {course.modules.length} Mô-đun
                        </span>
                        {isCompleted ? (
                          <button
                            onClick={() =>
                              alert(`Tải chứng chỉ PDF: ${course.title}`)
                            }
                          ...
                          <Link
                            ...
                            className="bg-[#0066FF] text-white font-bold text-xs px-3 py-1.5 rounded-lg hover:bg-blue-700 transition"
                          >
                            Vào học tiếp →
                          </Link>
                        )}
              ...
              <div className="text-center py-16 bg-white border border-dashed border-gray-200 rounded-2xl space-y-3">
                ...
                <button
                  onClick={() => setActiveTab("all-courses")}
                  className="text-xs bg-blue-50 text-[#0066FF] font-bold px-4 py-2 rounded-xl hover:bg-blue-100 transition"
                >
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h3 className="text-xs font-black uppercase text-gray-400 tracking-wider">
                  Bộ lọc từ khóa khám phá
                </h3>
                <p className="text-[11px] text-gray-500 mt-0.5">
                  Đang lọc hiển thị nhóm:{" "}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {allDisplayedCourses.map((course) => {
                  ...
                  return (
                    <div
                      key={course.id}
                      className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-col justify-between shadow-2xs hover:shadow-xs hover:border-blue-200 transition"
                    >
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="bg-blue-50 text-[#0066FF] text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-wide">
                            {course.category}
                          </span>
                          <span className="text-[10px] text-gray-400 font-bold">
                            {course.instructor}
                      ...
                      <div className="mt-6 pt-3 border-t border-gray-100 flex justify-between items-center">
                        ...
                        <Link
                          ...
                          className={`font-bold text-xs px-3 py-1.5 rounded-lg transition ${isEnrolled ? "bg-slate-100 text-slate-700 hover:bg-slate-200" : "bg-[#0066FF] text-white hover:bg-blue-700"}`}
                        >
    ...
    <Suspense
      fallback={
        <div className="p-10 text-center text-xs text-gray-400">
          Đang khởi tạo không gian học tập...
        </div>
      }
    >
      <HomeContent />
