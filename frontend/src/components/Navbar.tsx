    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50 px-6 py-3.5 flex justify-between items-center font-sans">
      ...
      <div className="flex items-center space-x-6">
        <Link
          ...
          className="text-base font-black text-[#0066FF] tracking-tight"
        >
          LUMER{" "}
          <span className="text-blue-400 font-medium text-xs">elearning</span>
        </Link>
            ...
            <div className="absolute left-0 mt-2 w-[550px] bg-white border border-gray-200 rounded-2xl shadow-xl p-5 grid grid-cols-2 gap-6 z-50">
              <div className="space-y-2">
                ...
                <div className="flex flex-col space-y-2 text-xs font-bold text-gray-600">
                  <span
                    onClick={() => handleCategoryClick("Khoa học Máy tính")}
                    className="hover:text-[#0066FF] cursor-pointer transition"
                  >
                    Khoa học Máy tính
                  </span>
                  <span
                    onClick={() => handleCategoryClick("An ninh mạng")}
                    className="hover:text-[#0066FF] cursor-pointer transition"
                  >
                    An ninh mạng
                  </span>
                  <span
                    onClick={() => handleCategoryClick("Phát triển Web")}
                    className="hover:text-[#0066FF] cursor-pointer transition"
                  >
                    Phát triển Web
                  </span>
                  <span
                    onClick={() => handleCategoryClick("Khoa học Dữ liệu & AI")}
                    className="hover:text-[#0066FF] cursor-pointer transition"
                  >
                    Khoa học Dữ liệu & AI
                  </span>
                  <span
                    onClick={() => handleCategoryClick("Thiết kế Đồ họa")}
                    className="hover:text-[#0066FF] cursor-pointer transition"
                  >
                    Thiết kế Đồ họa
                  </span>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col space-y-2 text-xs font-medium text-gray-500">
                  <span
                    onClick={() =>
                      handleCategoryClick("Google Career Certificates")
                    }
                    className="hover:text-gray-800 cursor-pointer transition"
                  >
                    Chứng chỉ Google Career
                  </span>
                  <span
                    onClick={() => handleCategoryClick("Infosec")}
                    className="hover:text-gray-800 cursor-pointer transition"
                  >
                    Hệ thống bảo mật Infosec
                  </span>
                  <span
                    onClick={() => handleCategoryClick("Princeton University")}
                    className="hover:text-gray-800 cursor-pointer transition"
                  >
                    Princeton Academic Program
                  </span>
                </div>
              </div>
            </div>
        ...
        {isLoggedIn ? (
          ...
          <>
            <Link
              ...
              className="text-xs font-bold text-gray-700 hover:text-blue-600 transition"
            >
              Đăng nhập
            </Link>
            <Link
              href="/login?mode=register"
              className="bg-blue-50 hover:bg-blue-100 text-[#0066FF] border border-blue-100 text-xs font-bold px-4 py-2 rounded-xl transition"
            >
              Đăng ký tài khoản
            </Link>
          </>
